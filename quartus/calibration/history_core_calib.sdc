# ---------------------------------------------------------------------------
# history_core_calib.sdc — timing constraints for the SPEC 18 corner-turn slice
...
# single-clock by construction. history_core has a separate history_clk domain in
# the real system, but this wrapper ties history_clk to core_clk (see
# history_core_wrap.sv, which states what that measures and what it does not), so
# the three cdc_ crossing primitives are still elaborated and still cost what
# they cost while the point stays a single-clock fabric measurement. Adding a
# second clock here would mean measuring something other than the kernel.
