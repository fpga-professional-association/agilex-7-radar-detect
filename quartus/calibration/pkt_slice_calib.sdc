# ---------------------------------------------------------------------------
# pkt_slice_calib.sdc — timing constraints for the SPEC 18 two-stage
# packet-fabric-slice calibration (issue #18).
#
# THE PROBE CONSTRAINT IS 600 MHz, NOT 400, and the reasoning is unchanged from
# quartus/calibration/cmult_calib.sdc — read that file for the full argument. In
# one paragraph: SPEC 8 constrains packet_clk at 400 MHz and
# quartus/constraints/clocks.sdc constrains the real design at exactly that. A
# Fitter that MEETS its constraint stops optimising, so a sweep constrained at
# the target measures the constraint rather than the kernel. The probe is
# therefore set deliberately above the achievable range, before any result is
# seen, in a project that is not the benchmark and whose output is a measurement
# rather than a claim. This is not SPEC 24's prohibition in reverse: SPEC 24
# forbids LOWERING a requested clock after seeing a poor result. The benchmark's
# own constraint is untouched and the exported record carries both numbers.
#
...
# No generated clocks and no asynchronous groups: the packet fabric is
# single-clock by construction, which `make cdc-inventory` proves on every
# regression (the packet build reports zero crossings under --strict). SPEC 8
# gives the network its own domain; the CROSSINGS into it belong to the
# multi-domain integration (issue #19) and are not part of this measurement.
# Adding a second clock here would mean measuring something other than the
# kernel.
