# ---------------------------------------------------------------------------
# bf_dot_calib.sdc — timing constraints for the SPEC 18 beamforming dot-product calibration
# (issue #12).
#
...
# single-clock by construction. The beamformer has a configuration domain in the
# real system, but this wrapper ties cfg_clk to core_clk (see bf_dot_wrap.sv, which has no configuration domain at all), so the
# crossing primitives are still elaborated and still cost what they cost while
# the point stays a single-clock fabric measurement. Adding a second clock here
# would mean measuring something other than the kernel.
