# ---------------------------------------------------------------------------
# fir_calib.sdc — timing constraints for the SPEC 18
# complex FIR lane (SPEC.md 18 item 2) calibration project (issue #10).
#
...
# The coefficient bank inside the wrapper has a configuration clock port, but
# fir_wrap ties it to the core clock, so the project is single-clock by
# construction and there is nothing here to declare asynchronous. The crossing
# primitives are still elaborated and still cost what they cost; what this point
# does not measure is a two-domain timing exercise, and it says so rather than
# hiding a real crossing behind a clock group (SPEC 24).
