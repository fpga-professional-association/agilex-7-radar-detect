...
## 2026-07-27 — Scenario-injection sim test tolerances  (issue #44, part 2)

**Context.** Part 2 of issue #44 injects a scenario generated by
`gen_target_iq.py` through `benchmark_pipeline_top` and checks the CFAR
detection stream against the scenario's ground-truth targets. Because
Phase 3 taps only ONE sample per beamformer beat (Decision 7 above)
the mapping between a scenario target and a CFAR event is constrained
in ways the reference-chain map (`range_doppler.py`) does not have to
respect. This entry logs the tolerances the sim test enforces, and
why. The test is `sim/tests/test_pipeline_scenario.cpp`, wired by
`make sim-scenario`.

**Decision 1 — Tapped-parity scenario.** With `BIN_PAR = 2` the align
network puts `LANE = fft_bin mod BIN_PAR`, i.e. lane 0 = even FFT bins
and lane 1 = odd bins. The pipeline taps beam 0, lane 0 of each
beamformer beat, so a target on an odd FFT bin is invisible to the
tap. `three_targets_even` (added to `SCENARIOS` in this PR) places
every target on an even FFT bin at range bins 40, 96, 160, mapping to
CFAR bins 20, 48, 80. The original `three_targets` scenario has a bin
at 67 (odd, invisible) and is not repurposed — the reference-map
picture-gate still uses `three_targets` (bin 67 is fine on the
reference chain, which sees every FFT bin), and `three_targets_even`
is the injection-gate scenario.

*Alternative rejected:* editing `three_targets` to shift its bins to
even values. That would silently change the picture reader knows and
break the reference-map golden thumbnails callers may cache locally.
A new scenario is cheaper than one you can't tell you didn't get.

**Decision 2 — Uniform-beam / angle 0 scenario targets.** With uniform
beamformer weights (bank-1 = 0x1000 per lane, the value
`Session::program_and_swap_to_active_banks` uses), coherent addition
across `N_ANT = 4` antennas nulls a target unless
`angle_idx = 0 (mod N_ANT)`. So `three_targets_even` sets every
target's `angle_idx = 0`. The Phase-3 sim test therefore cannot
distinguish beams; the reference map still overlays angle markers on
each antenna's plane, and Phase 5 (issue #20) is the follow-up where a
per-(beam, bin) fan-out and per-beam CFAR make angle discrimination
observable in RTL.

**Decision 3 — CFAR settings the injection test uses.**
`test_pipeline_scenario` configures the CFAR at guard = 1 lead / 1
lag, ref = 8 lead / 8 lag (16 references, half of the elaborated
`MAX_REF = 16` per side), alpha = 8.0 in UQ8.8 (`0x0800`), mode =
cell-averaging. These are the settings a Pfa ~= 1e-3 per bin per
frame implies at the noise floor `noise_sigma = 0.005` the generator
uses. With `HISTORY_FRAMES = 16` and ~110 fittable CFAR bins per
frame (the tapped path sees `FFT_SIZE / BIN_PAR = 128` bins, minus
the first and last `MAX_GUARD + MAX_REF = 18` that fall in the
suppressed window), the expected false-alarm count per scenario is
110 * 16 * 1e-3 ~= 1.8. Rounding for tail heaviness and seed
variability, the DOCUMENTED false-alarm bound is 6 across all frames
of the scenario. Measured on seeds 1, 2, 3: 1, 0, 1 false alarms
respectively — well under the bound.

*Alternative rejected:* raising alpha into the design-point 20-22
range. That is the correct choice for a true CFAR at Pfa 1e-6, but
the scenario's SNR spread is only 6-20 dB and the weakest target sits
too close to the noise floor for such a strict threshold — the weak
target would sometimes miss on a seed with a bright noise draw. The
weakened alpha is defensible for a synthetic scenario whose targets
are already known.

**Decision 4 — Bin tolerance ±1.** The sim test accepts a detection
at any CFAR bin within ±1 of the target's expected CFAR bin. Two
sources of ±1 shift add: (a) fixed-point quantisation of the pipeline
can nudge a bin's magnitude by one comparison step and let the
neighbour detect instead; (b) the CFAR window at the guard band's
inner edge is off by one from an idealized peak locator when the
guard = 1 side happens to capture the target's own leakage. The
reference chain (`range_doppler.py`) uses the same ±1 tolerance for
its own peak table; the two gates match by intention.

**Decision 5 — Detection aggregated across frames.** A range target's
magnitude at its bin is CONSTANT across frames (Doppler modulates
only phase, not magnitude). So a target detects at its bin on some
subset of frames — the exact subset depends on the per-frame noise
draw at neighbouring reference cells. The rule for "detected" is:
any frame in the run producing a DETECT within ±1 of the target's
expected CFAR bin. This is the natural cross-check with the
Phase-3 pipeline's per-frame detection loop; the alternative — every
frame must detect — would tighten the noise floor requirement without
adding a property the pipeline is supposed to guarantee.

**Decision 6 — Beam-check tolerance for Phase 3.** The sim test does
NOT check angle_idx. Beam 0 is the only tap in Phase 3
(Decision 7 of the earlier 2026-07-27 entry); every scenario target
has `angle_idx = 0` (Decision 2 above). The reference map's
antenna-overlay still checks angle, so the angle axis is covered by
the picture-gate; the sim-injection gate covers everything else.
Phase 5 (issue #20) is when the sim gate gains the beam axis.

**Follow-up.** When issue #20 lands the per-(beam, bin) fan-out and
per-beam CFAR, extend `test_pipeline_scenario` to (a) look for
DETECT events on the beam matching each target's `angle_idx` and
(b) put `three_targets` (angles 1/2/3, mixed-parity bins) back on
